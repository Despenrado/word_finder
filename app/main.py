...
async def upload_file(
    request: Request, file: UploadFile = File(...), search_word: str = "Quantori"
):
    ...
    try:
        is_word_exists, output_valid_file, output_invalid_file = (
            await file_service.process_file(file, search_word)
        )
        logger.info(
            f"results: {is_word_exists}, {output_valid_file}, {output_invalid_file}"
        )
        res = {"is_word_exists": is_word_exists}
        ...
        return templates.TemplateResponse("result.html", {"request": request, **res})
